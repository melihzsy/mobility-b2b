...
<body class="bg-slate-50 font-sans antialiased flex h-screen overflow-hidden">
    ...
    <aside class="w-64 bg-slate-900 text-white flex flex-col h-full shadow-2xl z-20">
        ...
        <nav class="flex-1 px-4 py-6 space-y-2 overflow-y-auto">
            ...
            <a href="{{ route('admin.categories.index') }}" class="block px-4 py-3 rounded-lg hover:bg-slate-800 text-slate-300 hover:text-white transition-colors">Category Management</a>
            <a href="{{ route('admin.products.index') }}" class="block px-4 py-3 rounded-lg hover:bg-slate-800 text-slate-300 hover:text-white transition-colors">OEM Parts</a>
        </nav>
    ...
    <main class="flex-1 flex flex-col h-full relative overflow-y-auto">
        <header class="h-16 bg-white shadow-sm flex items-center justify-between px-8 z-10 border-b border-slate-200">
            ...
            <div class="text-sm text-slate-500 flex items-center gap-2">
                ...
                <span class="font-medium text-slate-700">{{ auth()->user()->name ?? 'System Admin' }}</span>
            </div>
        </header>
